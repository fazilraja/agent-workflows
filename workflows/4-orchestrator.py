...
class BlogOrchestrator:
    ...
    def get_plan(self, topic: str, target_length: int, style: str) -> OrchestratorPlan:
        ...
        result = orchestratorAgent.run_sync(
            user_prompt=ORCHESTRATOR_PROMPT.format(
    ...
    def write_section(self, topic: str, section: SubTask) -> SectionContent:
        ...
        result = writerAgent.run_sync(
            user_prompt=WORKER_PROMPT.format(
                ...
                previous_sections_length=len(previous_sections) if previous_sections else 0,
            )
        )
        return result.data
    ...
    def review_post(self, topic: str, plan: OrchestratorPlan) -> ReviewFeedback:
        ...
        result = reviewerAgent.run_sync(
            user_prompt=REVIEWER_PROMPT.format(
                topic=topic, audience=plan.target_audience, sections=sections_text
            )
        )
        return result.data
